...
public class DetailServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if(action.equals("paper")) {
			...
			request.getRequestDispatcher("/detail_paperdetail.jsp").forward(request, response);
		} else if(action.equals("detail")) {
			try {
				ServiceFactory.getDetailServiceInstance().getQdetail(request);
			} catch (Exception e) {
				e.printStackTrace();
			}
			request.getRequestDispatcher("/detail_detail.jsp").forward(request, response);
		}
	}
